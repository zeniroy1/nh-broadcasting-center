...
class NhGeofenceService {
  ...
  static const double _dismissedExitThresholdMeters = 100;
  static const int _lowPowerIntervalMs = 300000;
  static const int _lowPowerAccuracyMeters = 150;
  static const int _lowPowerDelayMs = 30000;
  ...
  Future<void> _startInternal({
    ...
    if (_isRunning) {
      await _stopInternal(clearDismissal: false);
    }
    if (isPaused) {
    ...
    try {
      _geoService = GeofenceService.instance.setup(
        interval: _lowPowerIntervalMs,
        accuracy: _lowPowerAccuracyMeters,
        loiteringDelayMs: _lowPowerDelayMs,
        statusChangeDelayMs: _lowPowerDelayMs,
      ...
      LogFileService.log(
        '[NH알리미] 지오펜스 시작 — lat:$lat, lng:$lng, 반경:${radius}m, '
        'mode:저전력 베이스, interval:${_lowPowerIntervalMs ~/ 1000}초',
      );
    ...
    try {
      ...
      if (judgment.zone == LocationZone.reliableInside) {
        ...
        _startNhAppCheck();
          ...
          !(await _isReminderActive(prefs)) &&
          judgment.isInitialAlertCandidate) {
        LogFileService.log(
          '[NH알리미] 시작 시 초기 알림 후보 통과 → 즉시 알림 시작 '
          '(${judgment.initialAlertText}, zone:${judgment.zoneLabel})',
        );
        if (showUiEvent) {
          GeofenceUiEventService.show('지오펜스 근처 진입이 확인되어 알림 모니터링이 시작됩니다.');
        }
        final intervalSec = prefs.getInt('repeat_interval_sec') ??
            NotificationService.defaultRepeatIntervalSeconds;
        await _notif.stopReminder();
  ...
  Future<void> stop({
    ...
    bool? keepActiveReminder,
  }) async {
    return _enqueueOperation(
      () => _stopInternal(
        clearDismissal: clearDismissal,
        keepActiveReminder: keepActiveReminder,
      ),
    );
  }
  ...
  Future<void> _stopInternal({
    ...
    _stopNhAppCheck();
    final prefs = await SharedPreferences.getInstance();
    final shouldKeepReminder = keepActiveReminder ?? !clearDismissal;
    final keepReminder = shouldKeepReminder && await _isReminderActive(prefs);
    if (keepReminder) {
      _startNhAppCheck();
      LogFileService.log('[NH알리미] 지오펜스 재시작 중 — 반복 알림 보존');
    } else {
      await _notif.stopReminder();
    }
    if (clearDismissal) {
      ...
      LogFileService.log('[NH알리미] 지오펜스 재시작 준비 완료 — 차단 플래그 유지');
    }
  }
  ...
  Future<bool> _isWithinConfiguredRadius(SharedPreferences prefs) async {
    try {
      ...
      if (judgment.isInitialAlertCandidate) {
        LogFileService.log(
          '[NH알리미] ENTER 검증 초기 알림 후보 통과 — '
          '${judgment.initialAlertText}, zone:${judgment.zoneLabel}',
        );
        GeofenceUiEventService.show('지오펜스 근처 진입이 확인되었습니다.');
        return true;
      }
  ...
  Future<void> _verifyAndHandleExit() async {
    try {
      ...
      if (judgment.isOutside) {
        final notifActive = await _isReminderActive(prefs);
        final wasDismissedUntilExit = _dismissedUntilExit;

        if (notifActive && !_dismissedUntilExit) {
          await prefs.setInt('geofence_exit_count', 0);
          LogFileService.log(
              '[NH알리미] EXIT — 알림 활성 상태라 GPS 이탈 판정 무시 (${judgment.outsideText})');
        } else if (wasDismissedUntilExit) {
          if (judgment.distance > _dismissedExitThresholdMeters) {
            await _notif.stopReminder();
            _stopNhAppCheck();
            _dismissedUntilExit = false;
            await prefs.setBool('dismissed_until_exit', false);
            await prefs.setInt('geofence_exit_count', 0);
            LogFileService.log(
              '[NH알리미] EXIT — 출퇴근 확인 차단 해제 '
              '(${judgment.outsideText}, 차단 해제 기준 '
              '${_dismissedExitThresholdMeters.toStringAsFixed(0)}m 초과)',
            );
            GeofenceUiEventService.show('지오펜스 범위를 벗어났습니다.', isWarning: true);
          } else {
            await prefs.setInt('geofence_exit_count', 0);
            LogFileService.log(
              '[NH알리미] EXIT — 출퇴근 확인 차단 유지 '
              '(${judgment.outsideText}, 차단 해제 기준 '
              '${_dismissedExitThresholdMeters.toStringAsFixed(0)}m 초과)',
            );
          }
          ...
          await prefs.setInt('geofence_exit_count', 0);
          LogFileService.log(
              '[NH알리미] EXIT — 범위 밖 감지 (${judgment.outsideText})');
          GeofenceUiEventService.show('지오펜스 범위를 벗어났습니다.', isWarning: true);
        }
      } else {
        await prefs.setInt('geofence_exit_count', 0);
