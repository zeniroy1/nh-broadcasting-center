...
class NhBackgroundService : Service() {
    ...
    companion object {
        ...
        private const val NATIVE_CONFIRMED_EXIT_BUFFER_METERS = 120.0
        private const val NATIVE_MIN_CONFIRMED_EXIT_METERS = 150.0
    ...
    private fun shouldUseLastKnownForDismissedLowPower(
        ...
        val distanceToConfirmedExit =
            confirmedNativeExitThreshold(config.radius) - judgment.distance
    ...
    private fun confirmedNativeExitThreshold(radius: Double): Double {
        return max(radius + NATIVE_CONFIRMED_EXIT_BUFFER_METERS, NATIVE_MIN_CONFIRMED_EXIT_METERS)
    }

    private fun isConfirmedNativeExitAfterDismissal(radius: Double, judgment: NativeJudgment): Boolean {
        return judgment.distance > confirmedNativeExitThreshold(radius)
    }

    private fun dismissedLowPowerNativeDelay(radius: Double, judgment: NativeJudgment): Long {
        val distanceToConfirmedExit = confirmedNativeExitThreshold(radius) - judgment.distance
