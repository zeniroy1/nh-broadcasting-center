...
class NhBackgroundService : Service() {
    ...
    companion object {
        ...
        private const val NATIVE_DISMISSED_LOW_POWER_INTERVAL_MS = 180 * 1000L
        ...
        private const val NATIVE_ARRIVAL_GRACE_METERS = 20.0
        private const val NATIVE_ARRIVAL_GRACE_CONFIRM_REQUIRED = 2
        private const val NATIVE_STRONG_INSIDE_RATIO = 2.0 / 3.0
        private const val NATIVE_LOOSE_INSIDE_ACCURACY_METERS = 120.0
        private const val NATIVE_INSIDE_FALLBACK_MS = 60 * 1000L
        private const val NATIVE_INSIDE_FALLBACK_COUNT = 3
        private const val NATIVE_INSIDE_FALLBACK_CORE_RATIO = 0.85
        private const val NATIVE_DISMISSED_EXIT_BUFFER_METERS = 25.0
        private const val NATIVE_MIN_DISMISSED_EXIT_METERS = 60.0
        private const val NATIVE_DISMISSED_NEAR_CONFIRMED_EXIT_METERS = 30.0
        ...
        private const val DEV_LOG_MAX_BYTES = 8 * 1024 * 1024L
        private const val RELEASE_LOG_MAX_BYTES = 1024 * 1024L
    ...
    private var nativeArrivalGraceConfirmCount = 0
    private var nativeInsidePendingSinceMs = 0L
    private var nativeInsidePendingCount = 0
    private var nativeInsidePendingBestDistance = Double.MAX_VALUE
    ...
    private fun appendRuntimeLog(message: String) {
        if (!shouldPersistRuntimeLog(message)) return
        ...
        try {
            val dir = if (BuildConfig.DEBUG) {
                File("/storage/emulated/0/Download/nh_partners")
            } else {
                getExternalFilesDir(null) ?: filesDir
            }
            if (!dir.exists()) dir.mkdirs()
            val file = File(dir, "nh_reminder_runtime.txt")
            rotateRuntimeLogIfNeeded(file)
            file.appendText(line)
        } catch (e: Exception) {
            try {
                val fallbackDir = getExternalFilesDir(null) ?: filesDir
                val fallbackFile = File(fallbackDir, "nh_reminder_runtime.txt")
                rotateRuntimeLogIfNeeded(fallbackFile)
    ...
    private fun rotateRuntimeLogIfNeeded(file: File) {
        val maxBytes = if (BuildConfig.DEBUG) DEV_LOG_MAX_BYTES else RELEASE_LOG_MAX_BYTES
        if (!file.exists() || file.length() < maxBytes) return

        val parent = file.parentFile ?: return
        val backup = File(parent, "nh_reminder_runtime.old.txt")
        if (backup.exists()) backup.delete()
        file.renameTo(backup)
    }

    private fun shouldPersistRuntimeLog(message: String): Boolean {
        ...
        return keepKeywords.any { message.contains(it) }
    }
    ...
    private data class NativeJudgment(
        ...
        val deepReliableInside: Boolean,
        val arrivalGraceInside: Boolean
    ) {
        val isOutside: Boolean
            get() = zone == "outside" || zone == "farOutside"

        fun decisionText(radius: Double): String {
            val centerThreshold = max(10.0, radius * NATIVE_STRONG_INSIDE_RATIO)
            val accuracyThreshold = max(radius, 35.0)
            val result = if (reliableInside) "통과" else "보류"
            return "거리 ${distance.toInt()}m / 반경 ${radius.toInt()}m, " +
                    "정확도 ${accuracy.toInt()}m, 통과기준: 확실권 ${centerThreshold.toInt()}m 이내" +
                    "(정확도 ${NATIVE_LOOSE_INSIDE_ACCURACY_METERS.toInt()}m 이하) " +
                    "또는 경계권 정확도 ${accuracyThreshold.toInt()}m 이하, " +
    ...
    private fun startNativeMonitoring(initialDelayMs: Long) {
        ...
        nativeArrivalGraceConfirmCount = 0
        resetNativeInsidePending()
    ...
    private fun requestNativeLocation(config: NativeConfig, generation: Long) {
        ...
        if (shouldUseLastKnownForDismissedLowPower(config, bestLastKnown)) {
    ...
    private fun finishNativeLocationCheck(
        ...
        saveNativeJudgmentForFlutter(judgment, note)
        ...
        val dismissedLowPower = config.dismissedUntilExit &&
                !isConfirmedNativeExitAfterDismissal(config.radius, judgment)
        val dismissedLowPowerText =
            if (dismissedLowPower) ", 출퇴근 확인 차단 저전력" else ""

        var deferEntryPopup = false
        var bridgedEntryPopup = false
        var fallbackEntryPopup = false
        if (judgment.reliableInside &&
            lastNativeZone != "inside" &&
            !config.dismissedUntilExit &&
            !config.notifActive
        ) {
            if (judgment.deepReliableInside) {
                nativeArrivalGraceConfirmCount = 0
                resetNativeInsidePending()
                appendRuntimeLog(
                    "[NH알리미] 네이티브 보조 팝업 허용 — 도착권 중심 진입 " +
                            "(${judgment.decisionText(config.radius)})"
                )
            } else {
                nativeArrivalGraceConfirmCount += 1
                if (nativeArrivalGraceConfirmCount < NATIVE_ARRIVAL_GRACE_CONFIRM_REQUIRED) {
                    deferEntryPopup = true
                    appendRuntimeLog(
                        "[NH알리미] 네이티브 보조 팝업 보류 — 도착권 진입 1회, " +
                                "다음 확인 후 알림 판단 (${judgment.decisionText(config.radius)})"
                    )
                } else {
                    nativeArrivalGraceConfirmCount = 0
                    resetNativeInsidePending()
                    appendRuntimeLog(
                        "[NH알리미] 네이티브 보조 팝업 허용 — 도착권 2회 확인 " +
                                "(${judgment.decisionText(config.radius)})"
                    )
                }
            }
        } else if (judgment.arrivalGraceInside &&
            nativeArrivalGraceConfirmCount > 0 &&
            lastNativeZone != "inside" &&
            !config.dismissedUntilExit &&
            !config.notifActive
            ...
            if (nativeArrivalGraceConfirmCount < NATIVE_ARRIVAL_GRACE_CONFIRM_REQUIRED) {
                deferEntryPopup = true
                appendRuntimeLog(
                    "[NH알리미] 네이티브 보조 팝업 보류 — 도착권 후보 유지, " +
                            "다음 확인 후 알림 판단 (${judgment.decisionText(config.radius)})"
                )
            } else {
                nativeArrivalGraceConfirmCount = 0
                bridgedEntryPopup = true
                resetNativeInsidePending()
                appendRuntimeLog(
                    "[NH알리미] 네이티브 보조 팝업 허용 — 도착권 후보 유지 후 2회 확인 " +
                            "(${judgment.decisionText(config.radius)})"
                )
            }
        } else if (judgment.zone == "unreliableInside" &&
            lastNativeZone != "inside" &&
            !config.dismissedUntilExit &&
            !config.notifActive
        ) {
            nativeArrivalGraceConfirmCount = 0
            fallbackEntryPopup = shouldAllowNativeInsideFallback(config, judgment)
            if (fallbackEntryPopup) {
                appendRuntimeLog(
                    "[NH알리미] 네이티브 보조 팝업 허용 — 범위 안 보류 지속 fallback " +
                            "(${judgment.decisionText(config.radius)}, ${NATIVE_INSIDE_FALLBACK_MS / 1000}초 이상 확인)"
                )
            }
        } else {
            nativeArrivalGraceConfirmCount = 0
            if (!judgment.reliableInside && judgment.zone != "unreliableInside") {
                resetNativeInsidePending()
            }
        }
        ...
        lastNativeZone = when {
            (judgment.reliableInside && !deferEntryPopup) ||
                    bridgedEntryPopup ||
                    fallbackEntryPopup -> "inside"
        ...
        appendRuntimeLog(
            "[NH알리미] 네이티브 보조 감시 — $status " +
                    "(${judgment.decisionText(config.radius)}, age:${age}ms, source:$note$timeoutText$dismissedLowPowerText), " +
                    "다음:${delaySec}초"
        )
        scheduleNativeMonitoring(delayMs)
    }

    private fun shouldAllowNativeInsideFallback(
        config: NativeConfig,
        judgment: NativeJudgment
    ): Boolean {
        val now = SystemClock.elapsedRealtime()
        if (nativeInsidePendingSinceMs == 0L) {
            nativeInsidePendingSinceMs = now
        }
        nativeInsidePendingCount += 1
        if (judgment.distance < nativeInsidePendingBestDistance) {
            nativeInsidePendingBestDistance = judgment.distance
        }

        val elapsedMs = now - nativeInsidePendingSinceMs
        val fallbackCoreDistance = config.radius * NATIVE_INSIDE_FALLBACK_CORE_RATIO
        val closeEnough = nativeInsidePendingBestDistance <= fallbackCoreDistance
        val stableEnough = nativeInsidePendingCount >= NATIVE_INSIDE_FALLBACK_COUNT

        return elapsedMs >= NATIVE_INSIDE_FALLBACK_MS && (closeEnough || stableEnough)
    }

    private fun resetNativeInsidePending() {
        nativeInsidePendingSinceMs = 0L
        nativeInsidePendingCount = 0
        nativeInsidePendingBestDistance = Double.MAX_VALUE
    }

    private fun shouldUseLastKnownForDismissedLowPower(
        config: NativeConfig,
        location: Location?
    ): Boolean {
        if (!config.dismissedUntilExit || location == null) return false

        val ageMs = locationAgeMs(location)
        if (ageMs > NATIVE_DISMISSED_LAST_KNOWN_MAX_AGE_MS) return false
        if (!location.hasAccuracy()) return false
        if (location.accuracy.toDouble() > NATIVE_DISMISSED_LAST_KNOWN_MAX_ACCURACY_METERS) {
            return false
        }

        val judgment = judgeNativeLocation(config, location)
        if (isConfirmedNativeExitAfterDismissal(config.radius, judgment)) {
            return false
        }

        val distanceToConfirmedExit =
            dismissedNativeExitThreshold(config.radius) - judgment.distance
        return distanceToConfirmedExit > NATIVE_DISMISSED_NEAR_CONFIRMED_EXIT_METERS
    }
    ...
    private fun judgeNativeLocation(config: NativeConfig, location: Location): NativeJudgment {
        ...
        val centerThreshold = max(10.0, config.radius * NATIVE_STRONG_INSIDE_RATIO)
        val accuracyThreshold = max(config.radius, 35.0)
        val arrivalThreshold = config.radius + NATIVE_ARRIVAL_GRACE_METERS
        val exitThreshold = max(config.radius + 25.0, 60.0)
        val reliableCoreInside =
            distance <= centerThreshold &&
                    accuracy <= NATIVE_LOOSE_INSIDE_ACCURACY_METERS
        val reliableEdgeInside =
            distance > centerThreshold &&
                    distance <= config.radius &&
                    accuracy <= accuracyThreshold
        val reliableArrivalGrace =
            distance > config.radius &&
                    distance <= arrivalThreshold &&
                    accuracy <= accuracyThreshold
        val reliableInside = reliableCoreInside || reliableEdgeInside
        val zone = when {
        ...
        return NativeJudgment(
            ...
            reliableInside,
            reliableCoreInside,
            reliableArrivalGrace
        )
    }

    private fun saveNativeJudgmentForFlutter(judgment: NativeJudgment, source: String) {
        getSharedPreferences("FlutterSharedPreferences", Context.MODE_PRIVATE)
            .edit()
            .putString("flutter.last_location_zone", judgment.zone)
            .putString("flutter.last_location_source", "네이티브 보조 감시:$source")
            .putLong("flutter.last_location_judgment_ms", System.currentTimeMillis())
            .apply()
    }

    private fun dismissedNativeExitThreshold(radius: Double): Double {
        return max(radius + NATIVE_DISMISSED_EXIT_BUFFER_METERS, NATIVE_MIN_DISMISSED_EXIT_METERS)
    }

    private fun isConfirmedNativeExitAfterDismissal(radius: Double, judgment: NativeJudgment): Boolean {
        return judgment.distance > dismissedNativeExitThreshold(radius)
    }

    private fun dismissedLowPowerNativeDelay(radius: Double, judgment: NativeJudgment): Long {
        val distanceToConfirmedExit = dismissedNativeExitThreshold(radius) - judgment.distance
        return if (distanceToConfirmedExit <= NATIVE_DISMISSED_NEAR_CONFIRMED_EXIT_METERS) {
            NATIVE_DISMISSED_LOW_POWER_INTERVAL_MS
        } else {
            NATIVE_DISMISSED_STABLE_INTERVAL_MS
        }
    }
    ...
    private fun nextNativeDelay(config: NativeConfig, judgment: NativeJudgment): Long {
        if (config.dismissedUntilExit &&
            ...
            return dismissedLowPowerNativeDelay(config.radius, judgment)
        }

        val radius = config.radius
        val distance = judgment.distance
        val zone = judgment.zone
