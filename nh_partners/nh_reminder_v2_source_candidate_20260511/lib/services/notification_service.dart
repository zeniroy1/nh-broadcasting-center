...
class NotificationService {
  ...
  static const _duplicateClickWindowMs = 120000;
  ...
  Future<void> startReminder({
    ...
    final prefs = await SharedPreferences.getInstance();
    if (await _cancelReminderStartIfBlocked(prefs)) return;
    ...
    if (_isActive || now - startingAt < 5000) {
      ...
      LogFileService.log(
        '[NH알리미] 반복 알림 예약 보강 — 활성 상태 감지, '
        '$intervalSeconds초 후 반복 예약 복구',
      );
      return;
    }
  ...
  Future<void> showNotificationDirect() async {
    try {
      ...
      if (await NativeMonitorBridge.showReminderNotification()) {
        LogFileService.log(
          '[NH알리미] 네이티브 직접 알림 발송 완료 — 본문 클릭 Activity 경유 NH파트너스 연결',
        );
        return;
      }
  ...
  Future<void> _handleOpenNhResponse() async {
    ...
    if (_isRecentlyHandled(handledAtMs, lastHandledMs)) {
      ...
      await UsageStatsService().launchNhApp();
      return;
    }
  ...
  static bool shouldAllowReminderStart({
    required bool isPaused,
    required bool dismissedUntilExit,
  }) {
    return !isPaused && !dismissedUntilExit;
  }

  Future<bool> _cancelReminderStartIfBlocked(
    SharedPreferences prefs,
  ) async {
    await prefs.reload();
    if (shouldAllowReminderStart(
      isPaused: prefs.getBool('is_paused') ?? false,
    ...
    await AndroidAlarmManager.cancel(_notifAlarmId);
    await NativeMonitorBridge.syncNotificationActive(false);
    LogFileService.log('[NH알리미] 반복 알림 시작 생략 — 차단 상태');
    return true;
  }
