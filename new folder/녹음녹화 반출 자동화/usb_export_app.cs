...
namespace NHMediaExportTool
{
    ...
    class MainForm : Form
    {
        ...
        private CheckBox chkMoveToDiscard;
        #endregion
        ...
        private void FilterResults()
        {
            ...
            // 모든 폴더(일반, 지난자료, 폐기파일)를 이름 기준 내림차순(최신순) 정렬
            // C# 문자열 내림차순 특성상 일반폴더 -> #지난자료 -> ##폐기파일 순으로 그룹핑되며 각 그룹 내 최신 날짜가 상단에 옵니다.
            var matches = folderCache.Where(f => f.ToLower().Contains(keyword))
        ...
        private async void BtnExport_Click(object sender, EventArgs e)
        {
            ...
            bool shouldMoveToDiscard = chkMoveToDiscard != null && chkMoveToDiscard.Checked;

            // 실제 파일 복사 IO 워커
            await RunCopyProcessAsync(srcPath, destPath, cleanTarget, shouldMoveToDiscard);
            
            // 복사 완료 후 UI 활성화
            SetControlEnabledState(true);
        }

        private async Task RunCopyProcessAsync(string srcPath, string destPath, string cleanTarget, bool moveToDiscard)
        {
            await Task.Run(() =>
            {
                try
                {
                    ...
                    bool isMoved = false;
                    if (moveToDiscard && !srcPath.Contains("##폐기파일"))
                    {
                        try
                        {
                            string discardDir = Path.Combine(currentNetPath, "##폐기파일");
                            if (!Directory.Exists(discardDir)) Directory.CreateDirectory(discardDir);
                            
                            string discardDest = Path.Combine(discardDir, cleanTarget);
                            if (Directory.Exists(discardDest))
                            {
                                // 중복 시 이름에 타임스탬프 추가
                                discardDest += "_" + DateTime.Now.ToString("yyyyMMdd_HHmmss");
                            }
                            Directory.Move(srcPath, discardDest);
                            isMoved = true;
                        }
                        catch (Exception moveEx)
                        {
                            this.Invoke((MethodInvoker)delegate { 
                                MessageBox.Show("폐기폴더 이동 실패 (파일 사용 중이거나 권한 문제): " + moveEx.Message, "이동 오류", MessageBoxButtons.OK, MessageBoxIcon.Warning); 
                            });
                        }
                    }

                    // 내역을 로그 파일로 기록
                    AppendExportLog(cleanTarget, isMoved);

                    this.Invoke((MethodInvoker)delegate { 
                        lblStatus.Text = isMoved ? "✔️ 복사 및 원본 폐기폴더 이동 성공! USB 안전 제거 가능" : "✔️ 복사 및 로그 기록 완료! USB 안전 제거 가능"; 
                        lblStatus.ForeColor = Color.Green;
                        pbProgress.Value = 100;
                    });
                }
        ...
        private void AppendExportLog(string folderName, bool isMovedToDiscard)
        {
            try
            {
                string logFile = Path.Combine(currentNetPath, AppConfig.LogFileName);
                string status = isMovedToDiscard ? " (폐기폴더로 이동됨)" : "";
                string logEntry = string.Format("[{0}] 반출 항목: {1}{2}\r\n", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), folderName, status);
