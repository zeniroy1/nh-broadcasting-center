...
class NhBackgroundService : Service() {
    ...
    companion object {
        ...
        private const val REMINDER_CHANNEL_ID = "nh_reminder_high_v1"
        ...
        private const val NH_PARTNERS_PACKAGE = "com.vus.nhpthrm"
    ...
    private fun runNativeMonitorCheck() {
        if (nativeCheckRunning) {
            scheduleNativeMonitoring(NATIVE_INSIDE_INTERVAL_MS)
            return
        }

        val config = readNativeConfig()
        if (config.paused || !NativeServiceScheduler.isMonitoringHours()) {
            Log.d("NHAlimi", "네이티브 보조 감시 대기: paused=${config.paused}")
    ...
    private fun requestNativeLocation(config: NativeConfig) {
        val locationManager = getSystemService(Context.LOCATION_SERVICE) as LocationManager
        val bestLastKnown = bestLastKnownLocation(locationManager)
        val provider = selectProvider(locationManager)

        if (provider == null) {
            finishNativeLocationCheck(config, bestLastKnown, timedOut = true, note = "provider 없음")
            return
        }
        ...
        val timeout = Runnable {
            ...
            finishNativeLocationCheck(config, bestLastKnown, timedOut = true, note = "위치 요청 timeout")
        }
        ...
        listener = object : LocationListener {
            override fun onLocationChanged(location: Location) {
                ...
                finishNativeLocationCheck(config, location, timedOut = false, note = provider)
            }
        ...
        try {
            ...
            handler.removeCallbacks(timeout)
            finishNativeLocationCheck(config, bestLastKnown, timedOut = true, note = "위치 요청 실패:${e.message}")
        }
    }
    ...
    private fun showNativeEntryPopup(judgment: NativeJudgment) {
        ...
        val launchIntent = nhPartnersLaunchIntent()
        ...
        val notification = NotificationCompat.Builder(this, REMINDER_CHANNEL_ID)
            ...
            .addAction(0, "📲 NH파트너스 열기", actionPendingIntent)
            .build()
    ...
    private fun nhPartnersLaunchIntent(): Intent {
        val intent = packageManager.getLaunchIntentForPackage(NH_PARTNERS_PACKAGE)
        if (intent != null) {
            return intent
        }

        appendRuntimeLog("[NH알리미] NH파트너스 실행 인텐트 없음 — package:$NH_PARTNERS_PACKAGE")
