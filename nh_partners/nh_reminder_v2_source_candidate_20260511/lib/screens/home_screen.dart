...
class _HomeScreenState extends ConsumerState<HomeScreen>
  ...
  bool _exportingDiagnosticLog = false;
  bool _exportingDeveloperLog = false;
  int _currentIndex = 0;
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            isActive
                ? ((settings.geofenceLat.toStringAsFixed(4) ==
                            AppSettings.defaultGeofenceLat.toStringAsFixed(4) &&
                        settings.geofenceLng.toStringAsFixed(4) ==
                            AppSettings.defaultGeofenceLng.toStringAsFixed(4))
                  ...
                  IconButton(
                    icon: _exportingDiagnosticLog
                        ...
                        : const Icon(Icons.article_outlined,
                            color: Color(0xFF81D4FA)),
                    tooltip: '핵심 로그 TXT 내보내기',
                    onPressed:
                        _exportingDiagnosticLog ? null : _exportDiagnosticLog,
                  ),
                  if (kDebugMode)
                    IconButton(
                      icon: _exportingDeveloperLog
                          ? const SizedBox(
                              width: 20,
                              height: 20,
                              child: CircularProgressIndicator(
                          ...
                          : const Icon(Icons.bug_report_outlined,
                              color: Color(0xFFFFB74D)),
                      tooltip: '개발자 런타임 로그 TXT 내보내기',
                      onPressed:
                          _exportingDeveloperLog ? null : _exportDeveloperLog,
                    ),
                  IconButton(
        ...
        children: [
          ...
          _buildSettingsCard(
            child: ListTile(
              ...
              title:
                  const Text('권한 설정 관리', style: TextStyle(color: Colors.white)),
  ...
  Future<void> _showRadiusDialog(settings) async {
    double tempRadius = AppSettings.clampGeofenceRadius(
            ...
            content: SizedBox(
              ...
              child: Column(
                children: [
                  ...
                  Slider(
                    value: tempRadius,
                    min: AppSettings.minGeofenceRadius,
                    max: AppSettings.maxGeofenceRadius,
                    divisions: 3,
            ...
            actions: [
              ...
              TextButton(
                onPressed: () async {
                  ...
                  final updated = ref.read(settingsProvider);
                  final appliedRadius = updated.geofenceRadius.toInt();
                  if (!updated.isPaused) {
                    ...
                    messenger.showSnackBar(SnackBar(
                      content: Text('${appliedRadius}m 반경이 적용되었습니다.'),
                      backgroundColor: const Color(0xFF4DB6AC),
                    ));
                  } else {
                    messenger.showSnackBar(SnackBar(
                      content: Text(
                          '${appliedRadius}m 반경이 저장되었습니다. (모니터링 재개 시 적용됩니다.)'),
  ...
  Future<void> _calibrateLocation(settings) async {
    ...
    try {
      final result = await MonitoringRecoveryService.reset(
        ...
        clearDismissalOnResume: !settings.isPaused,
      );
      await ref.read(settingsProvider.notifier).reload();
      if (!result.positionUpdated && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              settings.isPaused
                  ? '위치 정확도가 낮아 기준점을 저장하지 않았습니다. 기존 위치와 모니터링 중지 상태를 유지했습니다.'
                  : '위치 정확도가 낮아 기준점을 저장하지 않았습니다. 기존 위치로 모니터링을 유지합니다.',
            ),
  ...
  Future<void> _resetToDefaultLocation() async {
    ...
    try {
      ...
      LogFileService.log(
        ...
        'lng:${updated.geofenceLng}, 반경:${updated.geofenceRadius}m, '
        '차단 플래그 해제:true',
      );
  ...
  Future<void> _exportDiagnosticLog() async {
    if (_exportingDiagnosticLog) return;

    setState(() => _exportingDiagnosticLog = true);
    try {
      final exported = await LogFileService.exportUserDiagnosticLog();
      await _shareLogExport(
        exported,
        shareText: 'NH알리미 핵심 동작 로그입니다.',
        snackbarText: 'TXT 로그 저장 완료',
      );
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('로그 내보내기 실패: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
      ...
      if (mounted) {
        setState(() => _exportingDiagnosticLog = false);
      }
    }
  }

  Future<void> _exportDeveloperLog() async {
    if (_exportingDeveloperLog) return;

    setState(() => _exportingDeveloperLog = true);
    try {
      final exported = await LogFileService.exportDeveloperRuntimeLog();
      await _shareLogExport(
        exported,
        shareText: 'NH알리미 개발자 런타임 전체 로그입니다.',
        snackbarText: '개발자 로그 저장 완료',
      );
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('개발자 로그 내보내기 실패: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
      ...
      if (mounted) {
        setState(() => _exportingDeveloperLog = false);
      }
    }
  }

  Future<void> _shareLogExport(
    UserDiagnosticLogExport exported, {
    required String shareText,
    required String snackbarText,
  }) async {
    if (!mounted) return;
    await Share.shareXFiles([XFile(exported.localPath)], text: shareText);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('$snackbarText: ${exported.savedPath}'),
        backgroundColor: const Color(0xFF4DB6AC),
      ),
    );
  }
